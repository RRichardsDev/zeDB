...
impl InputState {
    /// zeDB patch: re-run the completion trigger at the current cursor,
    /// as if the character before the cursor had just been typed. Lets
    /// the host app reopen completions when async context (e.g. schema
    /// metadata) arrives after the trigger character was typed.
    pub fn retrigger_completion(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let cursor = self.cursor();
        let text = self.text.to_string();
        let Some(last) = text[..cursor.min(text.len())].chars().next_back() else {
            return;
        };
